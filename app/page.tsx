...
const CAROUSEL_CONFIG = {
  ...
  leftColumnRatio: 'lg:w-2/5',
  rightColumnRatio: 'lg:w-3/5',
  slideSpeed: 3000, // অটো-প্লে স্পিড একটু বাড়ানো হয়েছে স্মুথনেসের জন্য
};
...
const ProductCardCarousel = () => {
  ...
  // নেভিগেশন হ্যান্ডলার (CSS Snap-এর সাথে পারফেক্ট কাজ করবে)
  const scroll = (direction: 'left' | 'right') => {
    if (!trackRef.current) return;
    
    const container = trackRef.current;
    const scrollAmount = container.clientWidth * 0.6; // প্রতি ক্লিকে ৬০% কন্টেইনার স্ক্রল হবে
  ...
  // অটো-প্লে ইফেক্ট (মোবাইলে টাচ করলে সাময়িক পজ হবে)
  useEffect(() => {
    const container = trackRef.current;
    if (!container) return;

    const autoPlay = setInterval(() => {
      // যদি একদম শেষে চলে যায়, শুরুতে ফেরত আসবে
    ...
    <section className={`py-20 ${CAROUSEL_CONFIG.backgroundColor}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
          ...
          {/* RIGHT COLUMN: Modern & Responsive Carousel */}
          <div className={`${CAROUSEL_CONFIG.rightColumnRatio} w-full relative group`}>
            ...
            <div
              ...
              style={{ scrollbarWidth: 'none' }} // Firefox এর জন্য স্ক্রলবার হাইড
            >
              {productImages.map((src, i) => (
                <div
                  key={i}
                  className="flex-none w-[200px] sm:w-[220px] h-[300px] sm:h-[340px] rounded-2xl overflow-hidden snap-center bg-white shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1"
                >
                  <img 
                    src={src} 
                    alt={`Product ${i + 1}`} 
            ...
            {/* Navigation Buttons - মোবাইলে হাইড থাকবে, ডেস্কটপে হোভার করলে দেখাবে */}
            <button
              onClick={() => scroll('left')}
              className="absolute left-2 top-1/2 -translate-y-1/2 w-11 h-11 rounded-full bg-black/70 hover:bg-black text-white hidden md:flex items-center justify-center backdrop-blur-sm transition-all opacity-0 group-hover:opacity-100 shadow-lg z-10"
              aria-label="Previous product"
            >
            ...
            <button
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 w-11 h-11 rounded-full bg-black/70 hover:bg-black text-white hidden md:flex items-center justify-center backdrop-blur-sm transition-all opacity-0 group-hover:opacity-100 shadow-lg z-10"
              aria-label="Next product"
            >
              <span className="text-lg font-bold">❯</span>
            </button>
...
// ... (বাকি Home Component এর কোড আগের মতোই থাকবে)
export default function Home() {
  return (
    <div className="min-h-screen">
      {/* Hero Section */}
